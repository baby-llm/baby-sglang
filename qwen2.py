...
from tp_runtime import tp_all_reduce_sum


def _get_tp_context() -> Tuple[int, int, Optional[object]]:
    if dist.is_available() and dist.is_initialized():
        tp_group = dist.group.WORLD
        tp_rank = dist.get_rank(group=tp_group)
        tp_size = dist.get_world_size(group=tp_group)
        return tp_rank, tp_size, tp_group

    tp_rank = int(os.getenv("RANK", "0"))
    tp_size = int(os.getenv("WORLD_SIZE", "1"))
    return tp_rank, tp_size, None
...
class QKVLinear(nn.Module):
    ...
    def __init__(
        ...
        if tp_size is None:
            _, tp_size, _ = _get_tp_context()
        self.tp_size = tp_size

        if self.total_num_heads % self.tp_size != 0:
            raise ValueError(
                f"num_heads must be divisible by tp_size: "
                f"num_heads={self.total_num_heads}, tp_size={self.tp_size}"
            )
        if self.total_num_kv_heads % self.tp_size != 0:
            raise ValueError(
                f"num_kv_heads must be divisible by tp_size: "
                f"num_kv_heads={self.total_num_kv_heads}, tp_size={self.tp_size}"
            )
        ...
        self.qkv_proj = nn.Linear(input_size, self.q_size + 2 * self.kv_size, bias=bias)
...
def _row_parallel_linear(
    ...
    if tp_size == 1:
        return linear(input_local)

    if not (dist.is_available() and dist.is_initialized() and tp_group is not None):
        raise RuntimeError(
            "Tensor parallel requires torch.distributed to be initialized. "
            "Call init_tp() before model forward when WORLD_SIZE > 1."
        )

    output_parallel = F.linear(input_local, linear.weight, None)

    output = tp_all_reduce_sum(output_parallel, tp_group)
...
class Qwen2Attention(nn.Module):
    ...
    def __init__(
        ...
        _, self.tp_size, self.tp_group = _get_tp_context()
...
class Qwen2MLP(nn.Module):
    def __init__(
        ...
        if hidden_act != "silu":
            raise ValueError(
                f"Unsupported activation: {hidden_act}. Only silu supported."
            )
        if intermediate_size % self.tp_size != 0:
            raise ValueError(
                f"intermediate_size must be divisible by tp_size: "
                f"intermediate_size={intermediate_size}, tp_size={self.tp_size}"
            )
        ...
        self.down_proj = nn.Linear(
            self.gate_up_proj.local_intermediate_size, hidden_size, bias=False
        )

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        gate, up = self.gate_up_proj(x)
        hidden_states = F.silu(gate) * up  # SwiGLU
...
class GateUpLinear(nn.Module):
    def __init__(
        ...
        if self.intermediate_size % self.tp_size != 0:
            raise ValueError(
                f"intermediate_size must be divisible by tp_size: "
                f"intermediate_size={self.intermediate_size}, tp_size={self.tp_size}"
            )
        self.local_intermediate_size = self.intermediate_size // self.tp_size
        self.gate_up_proj = nn.Linear(
            input_size, 2 * self.local_intermediate_size, bias=bias
        )
...
class BabyRadixAttention(nn.Module):
    ...
    def forward(
        self, q, k, v, forward_batch: SimplifiedForwardBatch, save_kv_cache: bool = True
    ):

        return SimpleAttentionBackend.forward(
